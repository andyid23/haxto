...
export class TimerMateriKuis extends I18NMixin(DDDSuper(LitElement)) {
  ...
  _onVisChange() {
    if (document.hidden) {
      ...
      if (this._sisaMateri <= 0 && !this._jalanMateri) {
        this.dispatchEvent(new CustomEvent("timer-materi-expired",{bubbles:true,composed:true}));
      }
      if (this._sisaKuis <= 0 && !this._jalanKuis) {
        this.dispatchEvent(new CustomEvent("timer-kuis-expired",{bubbles:true,composed:true}));
      }
    }
  }
