...
export class ModularQuiz extends I18NMixin(DDDSuper(LitElement)) {
  ...
  _startQuiz() {
    ...
    // Audit start kuis sumatif → db_aktivitas (bukan db_asesmen). Hanya untuk attempt
    // BARU (bukan resume dari localStorage) agar tidak menumpuk ganda. Diteruskan
    // ke parent dasbor-kuis yang meng-antre idempoten ke action=logActivity.
    if (!hasResumed && this.lockAfterComplete && this.studentId && this.kdMateri && this.kategori !== "formatif") {
      this._logActivity("timer_mulai", {
        kategori: this.kategori || "sumatif_lm",
        kdMateri: this.kdMateri,
        metadataKuis: this.judul,
        timestamp: new Date().toISOString(),
        durasiUlangan: this.timerDuration || this.duration || 300,
      });
    }
  }
  ...
  _pilihJawaban(indexKey, opsi) {
    ...
    // Edit jawaban yang sudah dikirim: kembalikan poin lama sebelum menambah yang baru.
  ...
  _toggleMultiAnswer(indexKey) {
    if (this._answered && !this.editable) return;
    const set = new Set(this._selectedAnswers);
    if (set.has(indexKey)) set.delete(indexKey);
    else set.add(indexKey);
    this._selectedAnswers = set;
  ...
  _submitMultiAnswers() {
    if (this._selectedAnswers.size === 0 || (!this.editable && (this._answered || this._userAnswers.has(this._currentIdx)))) return;
    const active = this._getActiveQuestions();
    const soal = this._normalisasiSoal(active[this._currentIdx]);
    const s = this._siapkanSoal(soal);
    // Edit jawaban yang sudah dikirim: kembalikan poin lama sebelum menambah yang baru.
...
// hanya "selesai_kuis" yang diteruskan ke parent/saldo akhir.
      ...
      return html`
        <div class="quiz-card result-box">
          ...
          ${this.mode === "guru" && !this.hidePauseRestart
            ? html`<button class="btn-start" style="background-color:var(--ddd-theme-secondary, rgba(0, 0, 0, 0.75));" @click=${() => {
                this._screen = "start";
                this.requestUpdate();
              }}>Ulangi Kuis</button>`
  ...
  _renderPGK(s) {
    ...
    return html`
              ...
              <tr>
                ...
                <td class="pgk-cell">
                  <input type="radio" name="pgk-${this._currentIdx}-${i}" value="true"
                    ?disabled=${this._answered && !this.editable} @change=${() => this._setPGK(i, true)}
                    aria-label="Pernyataan ${i + 1}: Benar" />
                </td>
                <td class="pgk-cell">
                  <input type="radio" name="pgk-${this._currentIdx}-${i}" value="false"
                    ?disabled=${this._answered && !this.editable} @change=${() => this._setPGK(i, false)}
      ...
      ${!this._answered || this.editable
        ? html`<button class="btn-submit" ?disabled=${this._answered && !this.editable} @click=${this._submitPGK}>Kirim Jawaban</button>`
        : ""}
    `;
  }
  ...
  _renderMatching(s) {
    ...
    return html`
        ...
        ${left.map(
          (item, i) => html`
            <div class="matching-row">
              ...
              <span>→</span>
              <select name="matching-${i}" class="matching-select" ?disabled=${this._answered && !this.editable}
                @change=${(e) => {
      ...
      ${!this._answered || this.editable
        ? html`<button class="btn-submit" ?disabled=${this._answered && !this.editable} @click=${this._submitMatching}>Kirim Jawaban</button>`
        : ""}
    `;
  }
        ...
        <div class="editor-content">
          <form class="add-question-form">
            ...
            ${this._tempQuestionImage
              ? html`<div style="text-align:center;margin:8px 0;"><img src=${this._tempQuestionImage} style="max-width:200px;border-radius:6px;border:1px solid var(--ddd-theme-polaris-border, var(--ddd-theme-default-limestoneLight, #e4e5e7));" alt="Pratinjau gambar soal" /></div>`
              : ""}
              ...
              <div class="question-card">
                ...
                ` : html`
                  <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;">
                    <div>
                      <strong style="color:var(--ddd-theme-primary, var(--ddd-primary-1, #1e407c));">[${(question.type || "mc").toUpperCase()}]</strong> ${question.question}
                      ${question.image ? html`<span style="font-size:11px;color:var(--ddd-theme-secondary, rgba(0, 0, 0, 0.75));">[gambar]</span>` : ""}
                      <span style="font-size:11px;color:var(--ddd-theme-success, var(--ddd-theme-default-success, #1e4620));font-weight:bold;">[${question.points || 1} poin]</span>
                    </div>
                    <div style="display:flex;gap:6px;">
                      <button class="btn-edit-soal" style="margin:0;padding:4px 10px;" @click=${() => this._startEditQuestion(index)}>✏️ Edit</button>
                      <button class="btn-edit-soal" style="margin:0;padding:4px 10px;background-color:var(--ddd-theme-error, var(--ddd-theme-default-error, #5f2120));" @click=${() => this._deleteQuestion(index)}>🗑️</button>
                    </div>
                  </div>
                `}
              </div>
  ...
  _renderEditorMC() {
    ...
    return html`
      ...
      ${this._tempCorrectAnswers.length <= 1
        ? html`<div style="font-size:11px;color:var(--ddd-theme-secondary, rgba(0, 0, 0, 0.75));margin-top:4px;">Centang 1 jawaban benar. Centang lebih dari 1 untuk mode PG Kompleks.</div>`
        : html`<div style="font-size:11px;color:var(--ddd-theme-primary, var(--ddd-primary-1, #1e407c));margin-top:4px;font-weight:bold;">Mode PG Kompleks: ${this._tempCorrectAnswers.length} jawaban benar dipilih</div>`}
    `;
  }
