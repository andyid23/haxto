      ...
      `]}render(){return this._selesai?d`
        <div class="wrap">
          <div class="selesai-card" role="status">
            ${this._habisWaktu?d`<div class="waktu">${this.pesanWaktuHabis}</div>`:$}
            ...
            ${this._skor!=null?d`<div class="skor">Skor Anda: <strong>${this._skor}%</strong></div>`:$}
            ${this.showSheetLink&&this.spreadsheetUrl?d`<a href="${this.spreadsheetUrl}" target="_blank" rel="noopener">📊 Buka Spreadsheet Nilai</a>`:$}
          </div>
        </div>
      `:d`
      <div class="wrap">
        <section class="materi-card">
          ${this.coverImage?d`<img class="cover" src="${this.coverImage}" alt="Cover materi" />`:$}
          <h2>📖 ${this.judulMateri}</h2>
          <p>${this.teksMateri}</p>
          <div class="materi-links">
            ${this.materiUrl?d`<a href="${this.materiUrl}" target="_blank" rel="noopener">${this.t.bacaMateri}</a>`:$}
            ${this.materiFile?d`<a href="${this.materiFile}" target="_blank" rel="noopener" download>${this.t.unduhMateri}</a>`:$}
          </div>
        </section>

        ${this._mulai?d`
              ${this._pesan?d`<p class="err-chip">${this._pesan}</p>`:$}
              <timer-kuis
            ...
            `:this.studentId?d`<button class="btn-mulai" @click="${this._mulaiLatihan}">${this.labelMulai}</button>`:document.querySelector("quiz-user-auth")?d`<p class="auth-hint">🔐 Silakan login lewat form di atas agar nilai tersimpan ke Spreadsheet.</p>`:d`
